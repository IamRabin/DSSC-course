...
void static_exec(const int N, int * a)
{
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp for schedule(static)//predetermined/predictable size of itetration to each thread
        for(int i = 0; i < N; ++i)
        {
            a[i] = thread_id;
        }
...
void static_exec_chunk(const int N, int * a, int chunk_size)
{
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp for schedule(static, chunk_size)///predetermined chunk size of itetration to each thread
        for(int i = 0; i < N; ++i)
        {
            a[i] = thread_id;
        }
...
void dynamic_exec(const int N, int * a)
{
    ...
    {
        int thread_id = omp_get_thread_num();
        #pragma omp for schedule(dynamic)//unpredictable variable work per iteration
        for(int i = 0; i < N; ++i)
        {
            a[i] = thread_id;
        }
...
void dynamic_exec_chunk(const int N, int * a, int chunk_size)
{
    ...
    {
        int thread_id = omp_get_thread_num();
        #pragma omp for schedule(dynamic, chunk_size)//each thread gets the chunk of task untill all iteration has been finished.
        for(int i = 0; i < N; ++i)
        {
            a[i] = thread_id;
        }
